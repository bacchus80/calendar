...
Personaly I prefer react-fetch-hook but I dont know if any external package is allowed.

API
From my understanding the external API endpoint dont support any filter so each API call
fetches all records. The filter takes place in the hook instead, so an external API call
is executed on every week load.
